...
# 无效 trial（(受试者, 序号)），对应测力台数据一并排除，不参与训练/评估：
# - LQW03/04（z1）：7 个 IMU 四元数整列全零（无 IMU 数据，2026-09-05 排查确认，
#   仅压力列正常）。
# - ZWJ10–20（z7）：左足鞋垫增益中途掉 ~26% 后恢复（2026-09-13 诊断确认）——
#   trial 10–20 左足压力峰 15.6→11.5 unit，而测力台左足 vy 峰 603→598 N 纹丝不动
#   （步态未变、力板读数正常，仅鞋垫硬件故障，典型移位/失接触）；右足鞋垫全程
#   稳定。该 11 个 trial 的「低压力↔正常力」矛盾样本会同时污染训练与评估
#   （z7 折左 vy 崩坏的直接根因）。用户 2026-09-13 拍板整 trial 排除。
# 总 trial 数 226 -> 224 -> 213。
